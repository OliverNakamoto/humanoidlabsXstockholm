    ...
    def _sync_read(
        ...
        *,
        ...
        for n_try in range(1 + num_retry):
            ...
            # print(f"DEBUG: Attempting sync read for IDs: {motor_ids} (Try {n_try + 1}/{1 + num_retry})")
   
            comm = self.sync_reader.txRxPacket()
            if self._is_comm_success(comm):
                # --- START DEBUGGING ADDITIONS ---
                # print(f"DEBUG: Sync read successful for IDs: {motor_ids} on try {n_try + 1}")
                # --- END DEBUGGING ADDITIONS ---
                break
            else:
                print(f"DEBUG: Sync read FAILED for IDs: {motor_ids} on try {n_try + 1}. Comm result: {comm} ({self.packet_handler.getTxRxResult(comm)})")
                # At this point, you know the group read failed. Now, let's try to ping each motor individually
                # to see which one is truly unresponsive. This will be slower, but diagnostic.
                print("DEBUG: Attempting to ping each motor individually to find the unresponsive one...")
                for motor_id in motor_ids:
                    ping_model_number, ping_comm, ping_error = self.packet_handler.ping(self.port_handler, motor_id)
                    if self._is_comm_success(ping_comm):
                        print(f"DEBUG: Motor ID {motor_id} responded to ping. Model: {ping_model_number}")
                    else:
                        print(f"DEBUG: Motor ID {motor_id} FAILED to respond to ping. Comm result: {ping_comm} ({self.packet_handler.getTxRxResult(ping_comm)})")
                        if ping_error != self._no_error:
                            print(f"DEBUG: Motor ID {motor_id} ping error: {self.packet_handler.getRxPacketError(ping_error)}")
                print("DEBUG: Individual motor ping check complete.")
            # --- END DEBUGGING ADDITIONS ---
