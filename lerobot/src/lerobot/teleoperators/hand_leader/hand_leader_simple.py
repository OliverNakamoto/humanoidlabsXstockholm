...
from .config_hand_leader import HandLeaderSimpleConfig
from .cv_hand_tracker_ipc import CVHandTrackerIPC
...
class HandLeaderSimple(Teleoperator):
    ...
    config_class = HandLeaderSimpleConfig
    name = "hand_leader_simple"

    def __init__(self, config: HandLeaderSimpleConfig):
        super().__init__(config)
        self.config = config

        # Initialize IPC tracker (connects to external hand tracking server)
        self.tracker = CVHandTrackerIPC(server_url="http://localhost:8888")
